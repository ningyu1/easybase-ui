<template>
  <div class="app-container">
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item :label="$t('warehouseTable.status')" prop="status">
                <el-select v-model="listQuery.status" :placeholder="$t('warehouseTable.status')" clearable style="width: 180px" class="filter-item">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="warehouseForm" :model="warehouseForm" :rules="rules" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('warehouseTable.code')" prop="code">
          <el-input v-model="warehouseForm.code" placeholder="输入编号" maxlength="32" />
        </el-form-item>
        <el-form-item :label="$t('warehouseTable.name')" prop="name">
          <el-input v-model="warehouseForm.name" placeholder="输入名称" maxlength="32" />
        </el-form-item>
        <el-form-item :label="$t('warehouseTable.status')" prop="status">
        ...
        <el-form-item :label="$t('warehouseTable.description')">
          <el-input v-model="warehouseForm.description" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="输入备注信息" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    createData() {
      this.$refs['warehouseForm'].validate(valid => {
        if (valid) {
          createWarehouse(this.warehouseForm).then(() => {
    ...
    handleUpdate(row) {
      this.resetWarehouseForm()
      this.warehouseForm = Object.assign({}, row) // copy obj
      this.warehouseForm.status = parseInt(this.warehouseForm.status)
    ...
    updateData() {
      this.$refs['warehouseForm'].validate(valid => {
        if (valid) {
          updateWarehouse(this.warehouseForm).then(() => {
            this.dialogFormVisible = false
            this.handleFilter()
            this.$message({
              message: '更新成功',
